 ...
 * </p>
 * At first it requires a valid path of folder/directory on which you want to
 * perform operations. Make sure the path is not system secure. It will
 * repetitively ask for a path till Path is not valid or Folder is not present.
 * 
 * @author Pranav Moreshwar Kale
 * @version 1.0
 * @since 26-April-2021
 */
...
public class FileHandler {
	...
	public static void main(String[] args) throws IOException, InputMismatchException {
		...
		System.out.printf(
				...
				" ");
		System.out.printf(
				"\n%15s<==============  This Project is developed by \"PRANAV KALE\"  ==============>\n",
				" ");
		changeFolder();
	}
	...
	static void printFile(List<File> file) throws IOException {
		...
		for (File i : file) {
			System.out.printf("| %-5s  | %-100s  |  %-20s | %-10s", ++j, i.getName(), typeOfFile(i),
					i.getAbsolutePath());
		}
		System.out.println(" ");
	}
	...
	/**
	 * The Method takes input of Path From User. <b>If The Path is Invalid it will
	 * warn and repetitively ask till the Path is valid.</b>
	 * 
	...
	public static void changeFolder() throws IOException, InputMismatchException {
		if (developer)
			System.out.println("Change Folder is Running...");
	 ...
	 * </p>
	 * <p>
	 * 3 )To change the current folder.
	 * </p>
	 * <p>
		...
		do {
			...
			try {
				if (choice < 1 || choice > 4)
					System.out.println("\t!!! Invalid choice. Please Enter valid choice between 1 and 4.!!!");
	...
	/**
	 * This Function display Operation Menu where user would have following three
	 * operations to perform on a File.
	 * <p>
	 * 1 )To Add a File/Folder.
	 * </p>
	 * <p>
	 * 2 )To Remove a File/Folder.
	 * </p>
	 * <p>
	 * 3 )Search For a File.
	 * </p>
	 * 
	 * @throws IOException If User give invalid Input or the File which does not
	 *                     exists.
	 */
	...
	/**
	 * When user choose option 1 from Operation Menu, this method is executed. In
	 * this Menu User have 2 option
	 * <p>
	 * 1 )To make new File.
	 * </p>
	 * <p>
	 * 2 )To make new Folder.
	 * </p>
	 * 
	 * @throws IOException
	 *                     <p>
	 *                     1 )For invalid Path or Invalid Input.
	 *                     </p>
	 *                     <p>
	 *                     2 )If the Access to path is denied due to security
	 *                     reasons.
	 *                     </p>
	 */
	public static void addFileOrDir() throws IOException, InputMismatchException {
		...
		if (choice == 1) {
			...
			if (done)
				...
				System.out.println(
						"\t !!! Unable to create folder. Sorry for the inconvenience but please first check if any security issues are there.");
		}
		operationMenu();
	}

	/**
	 * To create a new File. User have to give input as File name which he wants to
	 * create.
	 * <p>
	 * <b><i>The Method is Case-Sensitive.</i></b>
	 * </p>
	 * If the Folder/File (with same extension) with given input is already present
	 * in folder, file would not be created and it will return False on
	 * unsuccessful.
	 * 
	 * @return <b><i>Return True if the File is created SuccessFully else
	 *         False.</i></b>
	 */
	public static boolean addFile() {
		if (developer)
			System.out.println("Add File Running...");
		...
		if (newFile.exists())
			System.out.println("\t\t" + newFile + " --> Already Exists at " + newFile.getAbsolutePath());
		else {
			try {
				if (!newFile.exists()) {
					taskHappen = newFile.createNewFile();
				}
			} catch (Exception e) {
	...
	/**
	 * To create a new Folder. User have to give input as Folder name which he wants
	 * to create.
	 * <p>
	 * <b><i>The Method is Case-Sensitive.</i></b>
	 * </p>
	 * If the Folder/File (without extension) with same name is already present in
	 * folder, folder would not be created and it will return False on unsuccessful.
	 * 
	...
	public static boolean addFolder() {
		if (developer)
			System.out.println("Add Folder Running...");
		...
		try {
			if (!newFolder.exists()) {
				...
				System.out.println("\t\t" + newFolder + " Already Exists at --> " + newFolder.getAbsolutePath());
		} catch (Exception e) {
	...
	/**
	 * To Delete the user specified File/Folder. It will again ask for confirmation
	 * of Deleting the File/Folder by displaying the details of same.
	 * <p>
	 * <b><i>The Method is Case-Sensitive.</i></b>
	 * </p>
	 * 
	 * @throws IOException
	 *                     <p>
	 *                     1 )If the specified File/Folder is not Present
	 *                     </p>
	 *                     <p>
	 *                     2 )The Folder Have some security issues.
	 *                     </p>
	 *                     <p>
	 *                     3) the specified Folder is not Empty.
	 *                     </p>
	 *                     <p>
	 *                     4)Access is Denied.
	 *                     </p>
	...
	public static void deleteFile() throws IOException {
		if (developer)
			System.out.println("Delete File Running...");
		System.out.print(
				"\tSpecify The File Name to delete with its .dot extension else program will cause problem --> ");
		String FileName = stringreader();
		...
		if (taskHappen)
			...
			if (df.isDirectory())
				System.out.println(" \t!!! Directory must be empty to delete");
			else if (!df.exists())
				System.out
						.println("\t\t!!!There is no such file " + FileName + " in " + f.getAbsolutePath() + " Folder");
		}
		operationMenu();
	}
	 ...
	 * <p>
	 * If the searched path is folder, it asks user whether he want to change his
	 * current directory to searched one.
	 * </p>
	 * <p>
	 * <b><i>The Method is Case-Sensitive.</i></b>
	 * </p>
	...
	public static void searchFile() throws IOException {
		if (developer)
			System.out.println("Search File Running...");
		int j = 0;
	...
	/**
	 * Displays The Files in Current Folder in ascending order with respect to name
	 * of Files.
	 * 
	 * @throws IOException For Invalid Input
	 */
...
//            System.out.println("\t"+i);
		...
		if (!f.isDirectory())
			...
			System.out.println(" \t!!!! " + f.getAbsolutePath() + " Directory is empty");
		else {
			Arrays.sort(list);
			printFile(list);
		}
...
//    }
	...
	/**
	 * This is Merge Sort algorithm to sort the files in ascending order. It is used
	 * in displayFiles() method. It takes parameter as an array of String
	 *
